...
func (c *Client) applyIntents(ctx context.Context, set proxymodel.IntentSet) {
	...
	// runs when the orchestrator marked this a complete cert set (both the instant
	// push and the periodic tick do). Runs BEFORE the change-suppression gate so a
	// suppressed re-apply still converges the cert store.
