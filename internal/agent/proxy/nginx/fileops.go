...
type stagedFile struct {
	// dest is the live sites-available path the new content is written to.
	dest string
	...
	existed bool
	// enabledLink is the sites-enabled symlink this apply created, empty if none.
	// On rollback of a brand-new file the symlink is removed too so no dangling
	// activation survives.
	enabledLink string
...
// the subsequent nginx state is reported via Validate/health.
