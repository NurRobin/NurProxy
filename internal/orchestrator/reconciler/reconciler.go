...
func (r *Reconciler) ensureAgentAddressRecord(ctx context.Context, a *models.Agent, dnsProvider provider.Provider, provConfig json.RawMessage, recordType, ip, recordID string, persist func(string)) {
	...
	if recordID == "" {
		...
		if adopt := firstRecordOfType(existing, recordType); adopt != nil {
			...
			if sameRecordContent(adopt.Content, ip) {
				r.audit("agent", a.ID, action+"_adopted", fmt.Sprintf("adopted existing %s %s -> %s", recordType, a.FQDN, ip))
				...
				r.audit("agent", a.ID, action+"_update_failed", uErr.Error())
			} else {
				r.audit("agent", a.ID, action+"_adopted", fmt.Sprintf("adopted + corrected %s %s -> %s (was %s)", recordType, a.FQDN, ip, adopt.Content))
			}
			return
		}
		if cname := firstRecordOfType(existing, "CNAME"); cname != nil {
			...
			r.setAgentDNSError(a, msg)
			r.audit("agent", a.ID, action+"_conflict", msg)
			return
		}
		...
		if cErr != nil {
			...
			r.audit("agent", a.ID, action+"_create_failed", cErr.Error())
			return
		}
		persist(newID)
		r.audit("agent", a.ID, action+"_created", fmt.Sprintf("created %s %s -> %s", recordType, a.FQDN, ip))
		return
	}
	...
	r.audit("agent", a.ID, "ddns_updated", fmt.Sprintf("updated %s %s: %s -> %s", recordType, a.FQDN, existing.Content, ip))
}
